...
void Background::update(const TVector2D& mouse_pos, const TVector2D& bot_pos)
{
    ...
    if (!flag)
    {
        ...
        if (mouse_pos.x < bot_pos.x + game.MAX_WIDTH &&
                mouse_pos.x > bot_pos.x - game.MAX_WIDTH &&
                mouse_pos.y < bot_pos.y + game.MAX_HEIGHT &&
                mouse_pos.y > bot_pos.y - game.MAX_HEIGHT)
        {
...
void Background::draw(const Map& map)
{
    ...
    glVertex2f(map.leftoffs - game.MAX_WIDTH, -map.topoffs);
    glVertex2f(map.rightoffs + game.MAX_WIDTH, -map.topoffs);
    ...
    glVertex2f(map.rightoffs + game.MAX_WIDTH, -map.bottomoffs);
    glVertex2f(map.leftoffs - game.MAX_WIDTH, -map.bottomoffs);
    glEnd();
